...
function importedPath(payload: unknown): string | undefined {
  ...
  const event = payload as { movieFile?: { path?: unknown }; episodeFile?: { path?: unknown } };
  if (typeof event.movieFile?.path === 'string') return event.movieFile.path;
  if (typeof event.episodeFile?.path === 'string') return event.episodeFile.path;
  return undefined;
}

export async function POST(request: Request) {
  const raw = await request.text();
  if (raw.length > MAX_BODY) return NextResponse.json({ error: 'Webhook body too large' }, { status: 413 });
  const secret = process.env.WEBHOOK_SECRET;
  if (!secret) return NextResponse.json({ error: 'Webhooks are not configured' }, { status: 403 });
  ...
  if (!acceptWebhook(digest)) return NextResponse.json({ accepted: true, deduplicated: true });
  const path = importedPath(payload);
  if (!path) { audit('webhook', 'Authenticated webhook had no imported-file path'); return NextResponse.json({ accepted: true, queued: false }); }
