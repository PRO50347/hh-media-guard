...
Use Node 20+ and ffmpeg. Run `npm ci`, `npm run typecheck`, `npm run lint -- --max-warnings=0`, `npm test`, `npm run audit`, `npm run secrets`, `git diff --check`, and `MG_BUILD=1 npm run build`. Tests use temporary databases, generated media and mocks. Build `docker build -t hh-media-guard:dev-gate .`, then `bash scripts/browser-gate.sh` for production browser, health, non-root, startup-key rejection, shutdown and persistent restart tests. Also run `python3 scripts/check-unraid.py` and `bash scripts/unraid-gate.sh` for fresh 99:100 bind mounts, real mock connection tests, restart/recreation and permission-boundary diagnostics. The gates create and remove their own fixtures and internal networks. Do not substitute live service URLs or real media.
