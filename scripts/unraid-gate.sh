...
docker run --rm --network none --user 0 --entrypoint sh -v "$fixture_dir:/fixture" "$fixture_image" -ec 'echo generated-fixture > /fixture/media/proof; for source in sonarr radarr; do ffmpeg -v error -f lavfi -i sine=frequency=440:duration=1 -metadata:s:a:0 language=eng -c:a flac /fixture/media/$source.mka; done; chown -R 99:100 /fixture/config; chmod 755 /fixture/config; chown -R 345:456 /fixture/media; chmod 555 /fixture/media'
