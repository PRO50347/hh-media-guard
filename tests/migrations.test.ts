...
describe('migration serialization', () => {
  it('rolls back failed migrations and their version records', () => {
    ...
    expect(() => migrate(db, ['CREATE TABLE example(id INTEGER)', 'invalid SQL'])).toThrow();
    expect(db.prepare("SELECT name FROM sqlite_master WHERE name='example'").get()).toBeUndefined();
    db.close();
  });
  it('preserves v0.1 audit records while adding actors', () => {
    const db = new Database(':memory:');
    db.exec("CREATE TABLE events(id INTEGER PRIMARY KEY,type TEXT,detail TEXT,created_at TEXT); INSERT INTO events VALUES(1,'scan','fixture','2026-01-01')");
    migrate(db, ['CREATE TABLE IF NOT EXISTS events(id INTEGER PRIMARY KEY)']);
    expect(db.prepare('SELECT detail,actor FROM events').get()).toEqual({detail:'fixture',actor:'system'});
    migrate(db, ['CREATE TABLE IF NOT EXISTS events(id INTEGER PRIMARY KEY)']);
  ...
  it('serializes simultaneous startup migrations without duplicate ALTERs', async () => {
    ...
    await Promise.all(Array.from({length:8}, () => new Promise<void>((resolve,reject) => {
      const worker = new Worker(code, {eval:true,workerData:database});
      worker.on('error',reject);
      worker.on('exit',status => status === 0 ? resolve() : reject(new Error(`Worker exited ${status}`)));
    })));
