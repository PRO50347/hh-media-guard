...
describe("language evidence", () => {
  ...
  it("does not conflate descriptive audio with commentary", () => {
    expect(isCommentary("Audio Description")).toBe(false);
    expect(isDescriptive("Audio Description")).toBe(true);
  });
  it("always rejects commentary as main-program evidence", () =>
    expect(
      decideAudio(600, [track("eng", { isCommentary: true })], {
        ...defaults(),
        ignoreCommentary: false,
      }).decision,
    ).toBe("fail"));
  it("accepts descriptive audio only when configured", () => {
    ...
    expect(
      decideAudio(600, tracks, { ...defaults(), allowDescriptive: true })
        .decision,
    ).toBe("pass");
  });
  it.each([0, NaN, undefined])(
    "does not fail conclusively for invalid duration %s",
    (duration) =>
      expect(decideAudio(duration, [track("spa")], defaults()).decision).toBe(
        "needs-analysis",
      ),
  );
